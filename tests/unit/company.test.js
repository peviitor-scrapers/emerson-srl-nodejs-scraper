...
describe('company.js', () => {
  ...
  describe('getCompanyData (no cache)', () => {
    it('should fetch Emerson via direct CIF lookup and return company data', async () => {
      ...
      expect(result).toHaveProperty('company', 'EMERSON SRL');
      expect(result).toHaveProperty('cif', '18284762');
      expect(result).toHaveProperty('active', true);
      expect(result).toHaveProperty('anafData');
      expect(result.anafData.name).toBe('EMERSON SRL');
    });
    ...
    it('should fall back to company config when ANAF returns no company name', async () => {
      mockFetch.mockResolvedValueOnce(anafCompanyResponse({ cui: 18284762, name: null }));
  ...
  describe('getCompanyData (with cache)', () => {
    ...
    it('should use cached company data when available', async () => {
      ...
      expect(result.cif).toBe('18284762');
  ...
  describe('validateAndGetCompany', () => {
    ...
    it('should return company data with status active', async () => {
      mockFetch
        ...
        .mockResolvedValueOnce(peviitorResponse([{ company: 'EMERSON SRL' }]));
