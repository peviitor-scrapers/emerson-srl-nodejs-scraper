...
| `tests/company.json` | Mock ANAF company data for EMERSON used in unit tests |
| `tests/validate-emerson-jobs.js` | **Emerson-specific fast validator (used by CI).** HEAD requests only, hardcoded Emerson CIF. Called nightly by `automation-testing.yml`. Supports `--dry-run` and `--delete`. |
...
| `tests/e2e/scraper.test.js` | E2E tests - full pipeline with real Emerson (Oracle Cloud HCM) API, ANAF, and SOLR |
...
| `ROBOTS.md` | robots.txt analysis and scraping policy for Emerson (Oracle Cloud HCM) |
...
| `delete_request.json` | **Manual maintenance tool** — SOLR payload to delete ALL jobs for CIF 18284762. Use only when you need to wipe Emerson jobs from SOLR entirely. Run with: `curl --user "${SOLR_AUTH}" "https://solr.peviitor.ro/solr/job/update?commit=true" -H "Content-Type: application/json" -d @delete_request.json` |
...
- Full workflow: validate company (ANAF+Peviitor) → scrape Emerson (Oracle Cloud HCM) → transform → upsert SOLR → generate docs/jobs.md